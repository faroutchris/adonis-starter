...
export default class HttpExceptionHandler extends ExceptionHandler {
  ...
  async handle(error: unknown, ctx: HttpContext) {
    ...
    console.log(
      'validationManager.isValidationError(error), validationManager.isTurboEnabled()',
      ctx.validationManager.isValidationError(error),
      ctx.validationManager.isTurboEnabled()
    )

    // Handle validation errors for Turbo Stream requests using ValidationManager
    if (ctx.validationManager.isValidationError(error) && ctx.validationManager.isTurboEnabled()) {
      const turbo = await ctx.validationManager.handleValidationError(error)
